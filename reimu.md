...
Пакет программного обеспечения REIMU (REmote Intelligent Management Unit) предназначено для проведения действий с модулем на основе процессора Эльбрус-8С или Эльбрус-8СВ (далее «системный модуль», «система», «управляемая машина»). При этом системный модуль может быть выключен, достаточно подать на него STANDBY-питание.

ПО REIMU может устанавливаться как на интегрированный в материнскую плату менеджер AST2400 (в будущем планируется поддержка AST2500), так и на модуль МУС-А, вставляемый в специализированный разъём на материнской плате управляемой машины.
...
* Включение, выключение и перезагрузка системного модуля;
* Подключение к последовательному порту системы (Serial-over-LAN);
* Логирование данных последовательного порта;
* Работа с I²C-устройствами (сенсорами) системного модуля;
* Отслеживание и индикация состояния перегрева процессоров;
* Отслеживание и установка состояния UID;
* Перепрошивка программы начального старта системного модуля.
...
После подачи STANDBY-питания на плату её не рекомендуется включать вручную до полной загрузки менеджера (в случае такого включения системный модуль будет находиться под сбросом до момента готовности менеджера). Процесс загрузки индицируется соответствующим светодиодом (при его наличии) и состоит из следующих этапов:
...
Работа с REIMU осуществляется по последовательному порту МУС (обычно выведен на штыревой или поверхностно-контактный соединитель системного модуля, помеченный как MNGR CONSOLE, BMC SP или подобным образом), или по локальной сети (один или два интерфейса LAN 10/100 выведены на соединители задней панели системного модуля).

Параметры подключения к последовательной консоли МУС-А:
...
Начальные IP-и MAC-адреса интерфейсов МУС-А:
...
Если настройка какого-нибудь интерфейса не требуется (например, `eth1` на многих платах просто не выведен, и его настраивать смысла нет), то его нужно обозначить как unmamaged. Файл настройки в таком случае будет выглядеть как-то так (для примера, показан интерфейс `eth1`):
...
Техническая возможность не сбрасывать время при выключении питания менеджера (т.е. дежурного питания системного модуля) на данный момент не предусмотрена.
...
С удалённой машины также реализована возможность подключиться сразу к последовательному порту системного модуля (по умолчанию SSH на порт 2022). При этом будет произведено подключение к мультиплексору терминалов tmux, внутри которого запущен minicom, настроенный на работу с портом UART1 (`ttyS0`) менеджера, который на материнской плате соединён с портом `ttyS1` управляемой машины.

Для того, чтобы через последовательную консоль, предоставляемую REIMU, можно было войти в сеанс командной строки управляемой машины, на ОС Эльбрус необходимо убедиться, что в файле `/etc/inittab` присутствует строчка вида:

    s1:2345:respawn:/sbin/agetty -L ttyS1 115200

Если её там нет, то нужно добавить её в конец файла и перезагрузить систему.
...
Изменить порт прямого входа на COM-порт управляемой машины (по умолчанию: 2022) можно, изменив необходимый номер порта в трёх местах:
...
При подключении по SSH (на любой из сконфигурированных портов) будет запрошен пароль пользователя. Отключиться от cессии Serial-over-LAN можно нажатием клавиш Ctrl+B, отпустить, затем D (команда «`detach`» мультиплексора терминалов tmux). Подключаться может одновременно неограниченное количество пользователей, все пользователи имеют доступ как на ввод, так и вывод. Размер терминала выбирается наименьший из всех подключенных сессий.

Если пользователь уже подключен к менеджеру по SSH на обычный порт, то он может получить доступ к COM-порту управляемой машины, выполнив команду `tmux attach`. При отключении по «Ctrl+B D» он вернётся в свой шелл, откуда он выполнил указанную команду.

Локаль на всех консолях: `ru_RU.UTF-8`.

Лог данных из COM-порта управляемой машины по умолчанию сохраняется в файле `/var/log/sol.log`, хранящемся в оперативной памяти менеджера (при перезагрузке менеджера содержимое лога теряется). Лог автоматически сбрасывается в каталог `/usr/log` по достижении объёма 1 МБ (сохраняются три последних лога). Также в этом каталоге имеется ссылка на текущий лог с именем `current` и его сжатый бэкап `current.gz`, который пересоздаётся раз в минуту.
...
Логирование (если оно включено) ведётся даже в отсутствие подключенной SSH-сессии. Временно отключить (и затем включить) логирование в сессии можно, нажав Ctrl+A, отпустить, затем L. Файл не следует удалять (в таком случае Minicom потеряет к нему доступ); если его нужно вручную очистить, то для этого следует пользоваться командой `truncate`.

# Включение, выключение, перезагрузка системного модуля, работа с UID

Следующие команды SSH- или COM-консоли REIMU позволяют включать, выключать и перезагружать управляемую машину:

* `server_reset` – перезагрузка системы (нажатие кнопки сброса);
...
Возможно автоматическое включение питания управляемой машины при подаче standby-питания на изделие. Для этого в файле `/etc/auto_power_on` нужно заменить строчку «`AUTO_POWER_ON=off`» на «`AUTO_POWER_ON=on`».
...
| :----------------- | :-------------------: | :-------------------------------------------------------------------------------------------------------------------------------- |
| OVERHEATD_ENABLED  |           yes         | При значении «no» демон отключен (не запускается в процессе загрузки менеджера при включении STANDBY-питания материнской платы).  |
...
Данный файл позволяет автоматизированно удалённо определять параметры, относящиеся к работе системы (к примеру, пользовательская система контроля может собирать эти данные с управляемых хостов и агрегировать их для дальнейшего анализа администратором).
...
После успешной прошивки будет автоматически произведена перезагрузка менеджера (разумеется, это приведёт и к перезагрузке управляемой системы).

Прошивка с помощью данной утилиты не сбрасывает настройки пользователя. Если это необходимо, то нужно перед ссылкой на файл прошивки указать параметр `--fullclear`:

    firmware-updater --fullclear file:///tmp/fimware.bin

При возникновении ошибок файловой системы после перепрошивки (такое возможно при конфликте inode number-ов слоёв OverlayFS и подобных проблемах) рекомендуется сохранить важные файлы настроек с менеджера, перепрошить его с опцией `--fullclear`, а затем заново скопировать на него эти файлы.

При выкачивании прошивки по сети необходимо следить за тем, чтобы менеджер имел необходимый объём свободного ОЗУ размером чуть больше файла прошивки, поскольку прошивка скачивается во временный файл на RAM-диске менеджера. После прошивки этот файл удаляется. При этом исходный файл при работе по протоколу `file://`, разумеется, НЕ удаляется (такой файл не считается временным: раз пользователь сам скачал файл в файловую систему менеджера, то удалять его должен также он сам).
...
# Обновление программы начального старта системного модуля

Обновление программы начального старта (ПНС) системного модуля возможно с помощью программы-обёртки `update-host-flash` для утилиты `firmware-updater`.
...
Следует помнить, что на время прошивки ПНС SPI-шина с ПЗУ ПНС будет отключена от системного модуля (т.е. не рекомендуется производить прошивку ПНС, если системный модуль работает – это приведёт к его жёсткому сбросу).
