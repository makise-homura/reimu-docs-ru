...
В случае наличия корректного MAC-адреса в микросхеме энергонезависимой памяти FRU ID на материнской плате, при первой загрузке BMC его интерфейсам переназначаются следующие MAC-адреса (здесь `MAC` - MAC-адрес, прочитанный из этой микросхемы):

* Ethernet 0: `MAC + 00:00:00:00:00:3E`.
* Ethernet 1: `MAC + 00:00:00:00:00:3F`.

Для применения данных адресов BMC следует перезагрузить командой `reboot` (или снятием и повторной подачей STANDBY-питания).

Если в микросхеме FRU ID указанных данных нет, то MAC-адреса остаются неизменными.
