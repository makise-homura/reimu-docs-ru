...
ПО REIMU выпускается в виде следующих типов бинарных файлов (флаворов), предназначенных для прошивки в устройства:
...
* REIMU-4564: для прошивки в материнские платы, снабжённые ИС AST2400 с Flash-памятью размера 64 МБ и выводом в порт UART5.
* REIMU-5564: для прошивки в материнские платы, снабжённые ИС AST2500 с Flash-памятью размера 64 МБ и выводом в порт UART5.
...
* Ethernet 0: IP `192.168.1.1` (префикс 24), MAC 0: `98:A7:B0:00:00:01` (или автоматически определённый из FRU ID материнской платы при его наличии и корректности).
* Ethernet 1: IP `192.168.2.1` (префикс 24), MAC 1: `98:A7:B0:00:00:02` (или автоматически определённый из FRU ID материнской платы при его наличии и корректности).
...
Web-интерфейс менеджера доступен по протоколу https.
...
* `Overview` – позволяет посмотреть информацию о менеджере и состоянии платформы. На данный момент частично нефункционален.
* `Health – Event log` – на данный момент нефункционален.
* `Health – Hardware status` – на данный момент нефункционален.
* `Health – Sensors` – на данный момент нефункционален.
* `Control – Server power operations` – на данный момент нефункционален.
* `Control – Manage power usage` – на данный момент нефункционален.
...
* `Control – Serial over LAN console` – на данный момент нефункционален.
* `Control – KVM` – позволяет получить доступ к KVM-консоли (только на AST2500).
* `Control – Virtual Media` – позволяет подключить виртуальный носитель к платформе.
...
* `Configuration – Firmware` – на данный момент нефункционален.
...
В случае, если ПО REIMU запущено на микросхеме AST2500, возможно подключение по VNC к экрану `:0` на менеджере.

На данный момент функционал реализован в тестовом режиме без аутентификации. Если возможно несанкционированное подключение к VNC-серверу REIMU (например, менеджер включен в сеть, доступ к которой имеют не только администраторы), то рекомендуется с помощью `iptables` заблокировать все входящие подключения к порту 5900 на менеджере, кроме подключений с локального адреса (`127.0.0.1`).

В таком случае остаётся возможность использования функционала KVM-over-IP, если подключиться к менеджеру по SSH и пробросить локальный порт 5900 в клиентскую машину под тем же номером, затем с клиентской машины подключиться к экрану `locahost:0` (или другим вида `5900 + n`, но тогда подключаться надо будет к экрану `localhost:n`, здесь `n` – небольшое целое неотрицательное число).

Другой способ подключения – это зайти в web-интерфейс менеджера и воспользоваться пунктом меню `Control – KVM`. Он работает вне зависимости от блокировки порта.

## Media-over-IP

В случае, если ПО REIMU запущено на микросхеме AST2500, возможно подключение удалённых образов дисков к платформе.
