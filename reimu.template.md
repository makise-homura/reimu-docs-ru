...
* `Health – Sensors` – позволяет посмотреть показания сенсоров BMC и платформы. На данный момент там имеются только три сенсора BMC: загрузка процессора, использование памяти и использование верхнего оверлея файловой системы.
...
| WAIT_MSEC              |           2000        | Интервал (в мс) между опросами состояния процессоров.                                                                             |
...
* блок `cpus`, содержащий номера обнаруженных термодатчиков процессоров (от 0 до 3);
...
В ПО REIMU имеется файловый менеджер Midnight Commander версии 4.8.26. Запускается командой `mc`. Поддерживает подключение к удалённым файловым системам по протоколам FTP и SSH.
...
Также в ПО REIMU имеются стандартные диагностические утилиты и пакеты: `sysstat`, `htop`, `i2c-tools`; в 64-мегабайтных прошивках также присутствуют `tcpdump`, `ethtool`, `perf`, `ipmitool`, `iptables` и другие.
