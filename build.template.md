...
4) Должен иметься доступ в интернет по протоколам git, HTTP, HTTPS.

5) На хосте должна быть установлена программа `git` и другое необходимое сборочное окружение (`build-essentials`, `gcc`, `binutils`, `curl` и другое в зависимости от ОС).

6) Если хост находится за прокси-сервером, то этот прокси-сервер должен быть указан в настройках, доступных программам `git`, `wget`, `curl`. Также должны быть заданы (с помощью команды `git config`) следующие переменные:

```
url.https://github.com/.insteadof=git://github.com/
url.https://git.yoctoproject.org/git/.insteadof=git://git.yoctoproject.org/
url.https://git.savannah.gnu.org/git/.insteadof=git://git.savannah.gnu.org/
```
...
На хосте нежелательна установка пакетов `libfdt1` и `libfdt-dev` (или аналогичных им): в некоторых случаях это может привести к ошибкам сборки.
...
Также флавор REIMU-4564 соответствует изделию ТВГИ.00308-01, флавор REIMU-5564  — изделию ТВГИ.00306-01.
...
2) Войти в каталог с исходными текстами и переключиться на нужный тег (в примере `reimu-0.3`; вместо него можно поставить тег последнего релиза на гитхабе):

```
cd openbmc
git checkout reimu-1.0
```

Примечание — если исходники поставляются отдельным архивом, вместо пунктов 1 и 2 следует их распаковать в какой-нибудь каталог и перейти в него.

3) Настроить сборку на нужный флавор (в примере — REIMU-5564):
...
4) Запустить сборку образа прошивки из каталога, в который произведён переход предыдущей командой:

```
bitbake obmc-phosphor-image
```

5) Результатом сборки является файл с именем вида `obmc-phosphor-image-$FLAVOR-*.static.mtd`, лежащий в каталоге `tmp/deploy/images/$FLAVOR`, где `$FLAVOR` - имя выбранного флавора.

6) Сборка является успешной в случае появления указанного файла.

7) В случае необходимости начать сборку с нуля, необходимо удалить каталог, который стал текущим после выполнения п. 3, и повторить сборку, начиная с п. 3.
