...
1) Должны быть установлены программы `git` и `curl`.

2) Должно быть развёрнуто сборочное окружение: для Ubuntu 20.04 его разворачивание заключается в установке пакетов `gcc`, `g++`, `make`, `gawk`, `diffstat`, `chrpath`, `binutils`.
...
```
sudo apt install git curl gcc g++ make gawk diffstat chrpath binutils libfdt1 libfdt-dev
```
